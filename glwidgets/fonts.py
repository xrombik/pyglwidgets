...
class GlFreeType(object):
	...
	def __init__(self, file_name, font_height=glwidget.DEFAULT_FONT_SIZE, dpi=glwidget.DEFAULT_DPI, line_spacing=1.5):
		range_dl = 0
		for ordch in map(ord, GlFreeType.ALPHABET):
			if range_dl < ordch:
				range_dl = ordch
	...
	def draw_text(self, pos, s, color=colors.WHITE):
		# type: (GlFreeType, tuple, str, tuple) -> None
		"""
...
class GlFont(object):
	...
	def __new__(cls, face=glwidget.DEFAULT_FONT_FACE, face_size=glwidget.DEFAULT_FONT_SIZE, predraw=None):
		...
		font_item = GlFont.font_items.get((face, face_size))
		if font_item is not None:
			return font_item
		else:
			font_item = super(GlFont, cls).__new__(cls)
			GlFont.font_items[(face, face_size)] = font_item
