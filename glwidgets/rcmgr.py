...
class ResourceCtl(object):
	...
	def __init__(self, dir_local='', dir_user='', dir_prog='', data_path='data'):
		...
		if not os.path.exists(self.prog_save_path):
			os.mkdir(self.prog_save_path)
		print('Папка для хранения данных пользователя: %s' % self.user_save_path)
		print('Папка для хранения данных программы: %s' % self.prog_save_path)
	...
	def get_ft_font(self, font_file=DEFAULT_FREETYPE_FONT_FILE,
		...
		if font_key in self.ft_fonts:
			...
			ft_font = fonts.GlFreeType(path, font_size)
			print(u'\tвыполнено.')
